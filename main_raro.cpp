...
void get_clusters_at_rank(std::vector<std::string> labels, std::vector<std::vector<float>> distance_matrix, int rank, std::string mode)
{
    ...
    for (int k = 0; k < rank; k++)
    {
        ...
        file << "right_" << new_labels[min_col] << "[shape=point, width=0.0, height=0.0,label=\"\"];\n";
        file << "left_" << new_labels[min_col] << "[shape=point, width=0.0, height=0.0,label=\"\"];\n";
        file << "down_" << new_labels[min_col] << "[shape=point, width=0.0, height=0.0,label=\"\"];\n";


        file << labels[min_row] << "[label=\"" << labels[min_row] << "\"];\n";
        file << labels[min_col] << "[label=\"" << labels[min_col] << "\"];\n";

        file << new_labels[min_col] << " -- " << "down_" << new_labels[min_col] << ";\n";
        file << "down_" << new_labels[min_col] << " -- " << "left_" << new_labels[min_col] << ";\n";
        file << "right_" << new_labels[min_col] << " -- " << "down_" << new_labels[min_col] << ";\n";
        file << "left_" << new_labels[min_col] << " -- " << labels[min_row] << ";\n";
        file << "right_" << new_labels[min_col] << " -- " << labels[min_col] << ";\n";

        file << "{ rank=same; " << "down_" << new_labels[min_col] << "; " << "right_" << new_labels[min_col] << "; " << "left_" << new_labels[min_col] << ";}\n";
        file << "{ rank=same; " << labels[min_row] << "; " << labels[min_col] << ";}\n";
...
int main()
{
    ...
    get_clusters_at_rank(labels, dist_matrix, 5, "AVG");
